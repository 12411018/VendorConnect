...
class _AnalyticsPageState extends State<AnalyticsPage> {
  ...
  _AnalyticsData _buildAnalyticsData(
    ...
    final lowStockLines = <String>[];
    final highStockLines = <String>[];
    currentStock.forEach((name, qty) {
      if (qty <= 5) {
        lowStockLines.add('$name - $qty left');
      }
      if (qty >= 30) {
        highStockLines.add('$name - $qty available');
      }
    });

    final slowMovingLines = <String>[];
    for (final entry in currentStock.entries) {
      final sold = soldCounts[entry.key] ?? 0;
      if (entry.value >= 10 && sold <= 2) {
        slowMovingLines.add(
          '${entry.key} - ${entry.value} in stock, low movement',
        );
      }
    }

    final topProductLines = soldCounts.entries.toList()
      ..sort((left, right) => right.value.compareTo(left.value));
    final topRetailerLines = retailerCounts.entries.toList()
      ..sort((left, right) => right.value.compareTo(left.value));
    ...
    return _AnalyticsData(
      ...
      topProductLines: topProductLines
          .take(5)
          .map((entry) => '${entry.key} - ${entry.value} sold')
          .toList(),
      topRetailerLines: topRetailerLines
          .take(5)
          .map((entry) => '${entry.key} - ${entry.value} orders')
          .toList(),
      stockSummary:
          'Orders: $totalOrders, Revenue: ₹${totalRevenue.toStringAsFixed(0)}, Products: ${products.length}',
    );
  }
